...
const Login = () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.user?.role === 'admin') {
          ...
          navigate('/recruiter');
        } else if (result.user?.role === 'seeker') {
          navigate('/seeker');
        } else {
          navigate('/dashboard');
        }
  ...
  const inputFields = [
    { name: 'username', label: 'Username or Email', icon: '👤', type: 'text', placeholder: 'Enter your username or email' },
    { name: 'password', label: 'Password', icon: '🔒', type: showPassword ? 'text' : 'password', placeholder: 'Enter your password' },
  ];
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-12 bg-gray-50 dark:bg-gray-900 transition-colors duration-300 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        <motion.div
          ...
          className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-64 h-64 bg-purple-300/20 dark:bg-purple-600/10 rounded-full blur-3xl"
        />
      </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              ...
              >
                <label className="flex items-center cursor-pointer">
                  <input
                    type="checkbox"
                    className="w-5 h-5 rounded border-gray-300 dark:border-gray-600 text-violet-600 focus:ring-violet-500 bg-gray-50 dark:bg-gray-900"
                  />
